...
def perform_part_f():
    ...
    d, f = denoise_image('./pa4_data/noisy_10.txt', initialization = 'same', logfile = 'log_same')
    width = 1.0
    plt.clf()
    plt.bar(f.keys(), f.values(), width, color = 'b')
    plt.show()
    d, f = denoise_image('./pa4_data/noisy_20.txt', initialization = 'same', logfile = 'log_same')
    plt.clf()
    plt.bar(f.keys(), f.values(), width, color = 'b')
    plt.show()
